...
public protocol ReceivedClusteringManagerDelegate {
    
    func cellSizeFactorForCoordinator(coordinator:ReceivedClusteringManager) -> CGFloat
    
}

public class ReceivedClusteringManager : NSObject {
    
    public var delegate:ReceivedClusteringManagerDelegate? = nil
    ...
    public func clusteredAnnotationsWithinMapRect(rect:MKMapRect, withZoomScale zoomScale:Double) -> [MKAnnotation]{
        ...
        let cellSize:CGFloat = ReceivedClusteringManager.FBCellSizeForZoomScale(MKZoomScale(zoomScale))
        ...
        for i in minX...maxX {
            ...
            for j in minY...maxY {
                ...
                if count > 1 && zoomScale < self.maxZoomLevel {
                    ...
                    let cluster = ReceivedAnnotationCluster()
    ...
    public func displayAnnotations(annotations: [MKAnnotation], onMapView mapView:MKMapView){
        ...
        dispatch_async(dispatch_get_main_queue())  {
            ...
            for annotation in before {
                ...
                if !(annotation is ReceivedAnnotation || annotation is ReceivedAnnotationCluster) {
                    
                    before.removeObject(annotation)
                }
            }
    ...
    public class func FBCellSizeForZoomScale(zoomScale:MKZoomScale) -> CGFloat {
        ...
        let zoomLevel:Int = ReceivedClusteringManager.FBZoomScaleToZoomLevel(zoomScale)
