...
class FocusShape: UIView {
    ...
    internal func drawFocus(focusPoint: CGPoint) {
        ...
        let finalSize = CGSize(width: 35.0, height: 35.0)
        ...
        let finalShapeOrigin = CGPoint(x:-finalSize.width/2 + x, y: -finalSize.height/2 + y)
        ...
        easeIn.toValue = UIBezierPath(ovalInRect: CGRect(origin: finalShapeOrigin, size: finalSize)).CGPath
        easeIn.duration = 0.2
        ...
        easeOut.fromValue = UIBezierPath(ovalInRect: CGRect(origin: finalShapeOrigin, size: finalSize)).CGPath
